...
def main():
    try:
        logger.add(
            f"{server.path.log_path}/{datetime.today().strftime("%Y-%m-%d")}.log",
            rotation="1 day",
            retention="30 days",
            compression="zip",
            colorize=True,
        ...
        server.mods.data["server_base_mod"] = mod_loader.load_mod(
            f"{server.path.package_path}/server_base_mod"
        )

        try:
            server.mods.data |= mod_loader.load_mods(server.path.mod_path)
